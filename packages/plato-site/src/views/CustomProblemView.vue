...
.header {
  ...
  padding: 10px 16px; font-size: 15px; flex-shrink: 0;
...
.url-input {
  ...
  &:focus { border-color: var(--color-primary-hover); }
...
.or-divider {
  ...
  margin: 20px 0; font-size: 12px; color: var(--color-border-strong);
  &::before, &::after {
...
.error-msg { margin-top: 16px; font-size: 13px; color: #c44; }
...
%outline-btn {
  ...
  border-radius: 6px;
  &:hover { border-color: var(--color-primary-hover); color: var(--color-primary-hover); }
}

.spec-link {
  @extend %outline-btn;
  font-size: 12px; text-decoration: none; padding: 6px 14px;
  transition: color 0.15s, border-color 0.15s;
}

.back-btn { @extend %outline-btn; font-size: 13px; padding: 6px 20px; }
.stop-btn { @extend %outline-btn; font-size: 13px; padding: 6px 20px; }

// ── Footer ────────────────────────────────────────────────────
.footer {
  display: flex; align-items: center; gap: 8px;
  padding: 8px 12px; border-top: 1px solid var(--color-border); flex-shrink: 0;
}
.footer-info { flex: 1; text-align: center; font-size: 13px; color: var(--color-muted); }
...
.prove-desc {
  font-size: clamp(20px, 4vw, 32px); max-width: 700px;
...
.goal-label {
  font-size: 12px; font-weight: 600; letter-spacing: 0.08em; color: #aaa;
}
...
.victory {
  text-align: center; max-height: 90vh; overflow-y: auto; padding: 20px 16px;
}
.victory-text {
  font-size: clamp(28px, 5vw, 48px); font-weight: 400; margin-bottom: 16px;
}
.proof-scroll {
  max-height: 35vh; overflow-y: auto; text-align: left;
  font-size: 13px; line-height: 2; color: var(--color-subtle);
  padding: 16px 18px; margin: 0 auto 24px;
  background: var(--color-subtle-bg); border: 1px solid var(--color-border);
  border-radius: 8px; width: min(560px, 85vw); white-space: pre-wrap;
}
