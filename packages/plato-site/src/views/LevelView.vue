...
watch(
    ...
    ([sectionId, idx]) => {
        ...
        if (!sec) { router.replace({ name: 'notFound' }); return }
        if (!Number.isFinite(idx) || idx < 0 || idx >= sec.levels.length) {
            router.replace({ name: 'notFound' }); return
        }
        if (!progress.isUnlocked(sectionId, idx)) {
            const next = (progress.highestCompletedLevel[sectionId] ?? -1) + 1
            router.replace({ name: 'locked', query: { section: sectionId, n: String(idx), closest: String(next) } })
            return
        }
    },
    { immediate: true },
...
// Re-check when locale changes levels length
...
function goPrev() {
    router.push({ name: 'level', params: { sectionId: props.sectionId, idx: String(props.levelIdx - 1) } })
}
...
function goNextSection() {
    ...
    if (next) {
        router.push({ name: 'level', params: { sectionId: next.id, idx: '0' } })
    } else {
...
function onDiscoveryComplete() {
    ...
    discoveryStore.markViewed(props.sectionId, props.levelIdx)
    roadmap.add({
        sectionId: props.sectionId,
        sectionIdx: props.levelIdx,
        tag: 'discovery',
        idx: discoveryData.value?.lines.length ?? 0,
    })
    // Auto-advance to next level if unlocked
...
const proofLines = computed(() => {
    const entry = sectionEntries.value.find(e => e.sectionIdx === props.levelIdx && e.tag === 'problem')
...
function onSolved(steps: StepMeta[]) {
    ...
    if (problem.value) {
        ...
        // If this is the last level, next section opens after victory
...
const axiomSetLabel = computed(() => logicMode.value === 'pl' ? '(PL) FOL OFF' : '(FOL) FOL ON')
const axiomSetTitle = computed(() => logicMode.value === 'fol' ? 'FOL Axioms' : 'PL Axioms')
const allowedTactics = computed(() => section.value?.meta.allowedTactics ?? [])
</script>

<template>
    <div v-if="!section" class="not-found">{{ t('problem.notFound') }}</div>

    <!-- Discovery level -->
    <div v-else-if="isDiscovery" class="discovery-root">
        ...
        <div class="progress-track">
            <div class="progress-fill"
                :style="{ width: discoveryIdx / ((discoveryData?.lines.length ?? 1)) * 100 + '%' }">
            </div>
        </div>
        ...
        <div class="root">
            <NavBar @open-prefs="prefsOpen = true" @open-help="openHelp('commands')">
                <span class="goal-chip">
                    <span class="axiom-wrap" @click.stop="openHelp('notations')">
                        <span class="axiom-dot"
                            :class="{ 'axiom-dot--fol': logicMode === 'fol', 'axiom-dot--pl': logicMode === 'pl' }">
                        </span>
                        <span class="axiom-chip" :class="{ 'axiom-chip--fol-off': logicMode === 'pl' }">
                            {{ sectionName }} · {{ axiomSetLabel }}
                        </span>
                        <span class="axiom-pop">
                            <span class="axiom-pop-title" :class="{ 'axiom-pop-title--pl': logicMode === 'pl' }">
                                {{ axiomSetTitle }}
                            </span>
                            <span class="axiom-pop-row" v-if="logicMode === 'fol'">
                                <span class="axiom-pop-mark axiom-pop-mark--on">✓</span> Quantifiers
                            </span>
                            <span class="axiom-pop-row" v-else>
                                <span class="axiom-pop-mark axiom-pop-mark--off">✗</span> Quantifiers
                            </span>
                            <span class="axiom-pop-row" v-if="logicMode === 'pl'">
                                <span class="axiom-pop-mark axiom-pop-mark--on">✓</span> Modal
                            </span>
                            <span class="axiom-pop-row" v-else>
                                <span class="axiom-pop-mark axiom-pop-mark--off">✗</span> Modal
                            </span>
                        </span>
                    </span>
                </span>
            ...
            <div class="body">
                <button @click="goPrev" :disabled="!hasPrev" class="nav-btn nav-btn-top">
                    {{ t('problem.prev') }}
                </button>
                ...
                <Transition name="fade-in">
                    <ProofRepl v-if="agreed && showRepl" :goal-latex="goalLatex" :premise-latex="premiseLatex"
                        ...
                        :logic-mode="logicMode" :allowed-tactics="allowedTactics" @stepTaken="onStepTaken"
                ...
                <button @click="goNext" :disabled="!hasNext" class="nav-btn nav-btn-bot">
                    {{ t('problem.next') }}
                </button>
            </div>
...
.axiom-dot {
    ...
    &--fol { background: var(--color-fol-on); }
    &--pl { background: var(--color-fol-off); }
}
...
.axiom-pop-mark {
    &--on { color: var(--color-axiom-check); }
    &--off { color: var(--color-axiom-x); }
}
...
@media (max-width: 600px) {
    .goal-chip {
        .axiom-chip { display: none; }
        .axiom-dot { display: inline-block; }
