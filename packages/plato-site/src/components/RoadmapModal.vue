...
defineProps<{ totalProblems: number }>()
      ...
      <div class="rm-body">
        ...
        <div v-else class="rm-list">
          <div
            ...
            :key="entry.idx"
            class="rm-stop"
            :class="{ active: selected?.idx === entry.idx }"
          >
            <button class="rm-stop-btn" @click="selected = selected?.idx === entry.idx ? null : entry">
              <span class="rm-stop-num">{{ i + 1 }}</span>
              <span class="rm-stop-desc">{{ entry.description }}</span>
            </button>
            <div v-if="selected?.idx === entry.idx" class="rm-proof">
...
.rm-close:hover { color: var(--color-fg); }
.rm-body {
  overflow-y: auto; padding: 14px 20px 20px;
}
.rm-empty {
...
.rm-stop-btn {
  ...
  background: var(--color-subtle-bg); border: none;
  color: var(--color-fg);
  transition: background 0.15s;
}
.rm-stop-btn:hover { background: var(--color-border); }
.rm-stop-num {
  flex-shrink: 0;
