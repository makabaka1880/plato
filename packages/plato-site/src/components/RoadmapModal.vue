        ...
        <div class="rm-modal">
            ...
            <div class="rm-body">
                ...
                <template v-else v-for="section in visibleSections" :key="section.id">
                    ...
                    <div class="rm-list">
                        <div v-for="entry in (bySection[section.id] ?? []).sort((a, b) => a.sectionIdx - b.sectionIdx)"
                            :key="entry.sectionIdx" class="rm-stop"
                            :class="{ active: selected?.sectionId === entry.sectionId && selected?.sectionIdx === entry.sectionIdx }">
                            <button class="rm-stop-btn"
                                @click="selected = (selected?.sectionId === entry.sectionId && selected?.sectionIdx === entry.sectionIdx) ? null : entry">
                                <span class="rm-stop-num">{{ entry.tag == "problem" ? entry.sectionIdx : "🤔" }}</span>
                                <span class="rm-stop-desc">{{ problemDescription(entry.sectionId, entry.sectionIdx)
                                    }}</span>
                            </button>
                            <div v-if="selected?.sectionId === entry.sectionId && selected?.sectionIdx === entry.sectionIdx"
                                class="rm-proof">
                                <template v-if="entry.tag == 'problem'">
                                    <div class="rm-proof-label">{{ t('roadmap.proof') }}</div>
                                    <div v-for="(line, j) in proofLines(selected)" :key="j" class="rm-proof-line">
                                        <InlineLatex :text="line" />
                                    </div>
                                </template>
                                <template v-else>
                                    <div class="rm-proof-label">{{ t('roadmap.discovery') }}</div>
                                </template>
                            </div>
                        </div>
                    </div>
