            ...
            <template v-for="child in seg.children" :key="child.key">
                ...
                <b v-else-if="child.type === 'bold' && child.children">
                    <template v-for="c2 in child.children" :key="c2.key">
                        ...
                        <span v-else-if="c2.type === 'glossary'" class="glossary-link"
                            @click="emit('glossaryClick', c2.value)">{{ c2.display ?? c2.value }}</span>
                        <span v-else>{{ c2.value }}</span>
                    </template>
                </b>
                <code v-else-if="child.type === 'code'" class="inline-code">{{ child.value }}</code>
                <span v-else-if="child.type === 'glossary'" class="glossary-link"
                    @click="emit('glossaryClick', child.value)">{{
                        child.display ?? child.value }}</span>
                <span v-else>{{ child.value }}</span>
            </template>
        </b>
        <code v-else-if="seg.type === 'code'" class="inline-code">{{ seg.value }}</code>
        <span v-else-if="seg.type === 'glossary'" class="glossary-link" @click="emit('glossaryClick', seg.value)">{{
            seg.display
            ?? seg.value }}</span>
