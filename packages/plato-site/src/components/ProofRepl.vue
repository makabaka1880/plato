    ...
    <div v-else class="repl">
        ...
        <div v-if="prefs.viewMode === 'tex'" class="judgements">
            <div v-for="(e, i) in entries" :key="i" class="entry">
                <div class="entry-row">
                    ...
                    <div v-if="e.meta" class="nlg">
                        <InlineLatex :text="nlgText(e.meta)" @glossary-click="onGlossaryClick" />
                    </div>
                </div>
        ...
        <div class="input-wrap">
            <div v-if="liveParse.liveMeta.value" class="live-preview">
                ...
                <span v-if="liveParse.liveMeta.value.params.F" class="live-param">
                    <InlineLatex :text="'$F$: $' + liveParse.liveMeta.value.params.F + '$'" />
                </span>
                <template v-for="(latex, key) in liveParse.liveMeta.value.stepPreviews" :key="key">
                    <span class="live-param">
                        <InlineLatex :text="'$' + key + '$: $' + latex + '$'" />
                    </span>
                </template>
            </div>
            <div v-else-if="liveParse.parseError.value" class="live-preview live-err">{{ liveParse.parseError.value }}
            </div>
            <div class="input-row">
                <div v-if="autocomplete.visible.value" class="ac-dropdown">
                    <div v-for="(s, i) in autocomplete.suggestions.value.slice(0, 6)" :key="s.label" class="ac-item"
                        :class="{ active: i === autocomplete.activeIndex.value, kindFormula: s.kind === 'formula', kindStep: s.kind === 'step' }"
                        @click="autocomplete.activeIndex.value = i; autocomplete.accept()" @mousedown.prevent>
                        <span class="ac-label">{{ s.label }}</span>
                        <span v-if="s.latex" class="ac-desc">
                            <Katex :expr="s.latex" />
                        </span>
                        <span v-else class="ac-desc">{{ s.description }}</span>
                    </div>
                </div>
                <input ref="inpEl" v-model="input" @keydown="onKeydown" @click="autocomplete.dismiss()"
                    :disabled="guideInputDisabled" :placeholder="visibleGuide?.tactic ?? t('repl.placeholder')"
                    class="repl-input" />
                <button class="submit-btn" @click="run" :disabled="guideInputDisabled || !!liveParse.parseError.value"
        ...
        <HelpModal v-if="showHelp" :glossary-term="glossaryTerm ?? undefined" :allowed-tactics="props.allowedTactics"
