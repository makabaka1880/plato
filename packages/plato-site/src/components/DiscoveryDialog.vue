...
function speakerColorHsl(name: string): { h: number; s: number; l: number } {
    let hash = 0;
    ...
    const val = Math.abs(hash) * PHI;
    const fractionalPart = val - Math.floor(val);

    const h = Math.round(fractionalPart * 360);

    return { h, s: 65, l: 45 };
}

function speakerColor(name: string): string {
    const { h, s, l } = speakerColorHsl(name);
    return `hsl(${h}, ${s}%, ${l}%)`;
}

function speakerBgColor(name: string): string {
    const { h, s, l } = speakerColorHsl(name);
    return `hsl(${h}, ${s - 15}%, 95%)`;
}

// ── Single-card model with typewriter ───────────────────────────
const TYPING_SPEED = 28
const savedPos = discoveryStore.getPosition(props.sectionId, props.levelIdx)
...
watch(currentIdx, (val) => {
    emit('progress', val)
    discoveryStore.setPosition(props.sectionId, props.levelIdx, val)
}, { immediate: true })
...
<template>
    <div class="discovery-dialog" tabindex="0" @keydown="onKeydown">
        ...
        <div class="stage">
            ...
            <button class="nav-btn nav-flank nav-flank-top" :disabled="currentIdx <= 0" @click.stop="goPrev"
                title="Previous">▲</button>

            <div class="stage-inner">
                <Transition name="card-fade" mode="out-in">
                    <div v-if="currentLine" :key="currentIdx" class="card"
                        :style="{ backgroundColor: speakerBgColor(currentLine.sid) }">
                        <div class="card-speaker" :style="{ color: speakerColor(currentLine.sid) }">
                            {{ currentLine.speaker }}
                        </div>
                        <div class="card-body">
                ...
                <div class="card-nav">
                    <button class="nav-btn nav-side" :disabled="currentIdx <= 0" @click.stop="goPrev"
                        title="Previous">▲</button>
                    <button class="nav-btn nav-side" :disabled="currentIdx >= total - 1" @click.stop="advance"
                        title="Next">▼</button>
                </div>
            </div>

            <button class="nav-btn nav-flank nav-flank-bot" :disabled="currentIdx >= total - 1" @click.stop="advance"
                title="Next">▼</button>
        </div>
    ...
    <HelpModal v-if="showHelp" :glossary-term="glossaryTerm ?? undefined"
        @close="showHelp = false; glossaryTerm = null" />
</template>
